...
include_once LIBELLESROOT."/DB/db.php";
...
function alreadyExists($value,$pdo){
    $sql = "SELECT libelle FROM theme where libelle =:libelle";
...
function latests_themes()
{
    ...
    $themes = $pdo->query("SELECT * FROM theme ORDER BY theme_id DESC")->fetchAll(PDO::FETCH_OBJ);
    return $themes; // return themes au Controller qui sera utilisé par la views (for each $themes as $theme)
...
function create($libelle)//function qui ajoute des libelles en récupérant les champs remplis
{
    ...
    $sql = "INSERT INTO theme (theme_id, libelle) VALUES (null, :libelle)";
    $query=$pdo->prepare($sql);
...
function edit($id, $libelle){//edite les valeurs extraites de la fonction updateAction() du fichier controller qui a récupéré  les valeurs $_POST 
    ...
    $query=$pdo->prepare("UPDATE theme 
                            SET libelle =:libelle 
                            WHERE theme_id =:id");
    $query->bindParam(":libelle", $libelle, PDO::PARAM_STR);
    $query->bindParam(":id", $id, PDO::PARAM_INT);
