<?php
$title = 'Associer un plat à un menu';
...
<form action="index.php?action=store" method="post" enctype="multipart/form-data"><!--Les données seront envoyées dans store.php (action = store.php)-->
	<div class="selectList">
		...
		<label for="menus" class="label">
			<p><strong>1. Sélectionner un Menu</strong></p>
		</label>
		...
		<select name="menus" id="selectorMenus"> <!--onclick="getMenusJS(this.value)"-->
			<option class="none" disabled selected>Menu</option><!--la première option de la liste déroulante avec l'invitation à sélectionner-->
			<optgroup label="Sélectionner">
				<!--Les régimes de la liste sont fetch de la DB en utilisant la function du model getMenus()-->
				<?php
				...
				?>
					...
					<option id="optionDBmenu" class="database" value=<?php echo $menu['menu_id'] ?>><?= $menu['titre'] ?></option>
				<?php
				}
				?>
			</optgroup>
		...
		<div class="selectedMenu" id="startSelectedMenu">

		</div>
		<p class="requirement"><strong>Note: Pour ajouter un menu à la liste</strong> <br> → Veuillez vous rendre sur l'onglet:&nbsp;&nbsp; "Gestion"> "Menus" depuis la page d'accueil</p>

		<!--Liste Déroulante Plat -->
		<label for="plats" class="label">
			<p><strong>2. Sélectionner un plat</strong></p>
		</label>
		<select name="plats" id="selectorPlats" value=<?php if (!['plat_id']) {
															echo "";
														} else {
															echo $plat['plat_id'];
														} ?>> <!--onclick="getPlatJS(this.value)"-->
		...
		<p class="requirement"><strong>Note: Pour ajouter un plat à la liste:</strong> <br>
			→ Vous rendre sur l'onglet: &nbsp;&nbsp; "Gestion"> "Plats" depuis la page d'accueil</p>
	</div>
