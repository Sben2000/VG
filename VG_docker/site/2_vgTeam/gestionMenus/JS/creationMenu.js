...
myForm.addEventListener('submit', function (event) {
	...
	if (previewImage.style.display != 'block') {
		errorMessage.innerHTML =
			'Une image respectant les caratéristiques doit être chargée ';
		return false;
	}
	...
	if (menuTitleCleanValue.length < 5 || menuTitleCleanValue.length > 30) {
		errorMessage.innerHTML =
			'Le titre doit comporter entre 5 et 30 caractères ';
		return false;
	}
	...
	if (textInputCleanValue.length < 10 || textInputCleanValue.length > 500) {
		errorMessage.innerHTML =
			'La description doit comporter entre 10 et 500 caractères ';
		return false;
	}
	...
	if (
		minPeopleCleanValue * 2 == 'NaN' ||
		minPeopleCleanValue == '' ||
		minPeopleCleanValue * 2 < 0
	) {
		errorMessage.innerHTML =
			'Veuillez entrer un nbre de pers.min  au format mentionné entre 00 et 500 ';
		return false;
	}
	...
	// si la valeur calculée est de type "NaN" ou si la valeur est vide ou si la valeur est négative
	if (
		remainQtyCleanValue * 2 == 'NaN' ||
		remainQtyCleanValue == '' ||
		remainQtyCleanValue * 2 < 0
	) {
		errorMessage.innerHTML =
			'Veuillez entrer une quantité au format mentionné entre 00 et 500 ';
		return false;
	}
	...
	// si la valeur calculée est de type "NaN" ou si la valeur est vide ou si la valeur est négative
	if (
		menuPriceCleanValue * 2 == 'NaN' ||
		menuPriceCleanValue == '' ||
		menuPriceCleanValue * 2 < 0
	) {
		errorMessage.innerHTML =
			'Veuillez entrer un prix au format mentionné entre 0.00 et 500.00 ';
		return false;
	}
	...
	const regex1 =
		/["<>]/gs; /*Note: normalement flag g pour matcher toutes les instances (i si besoin Lower+Uppercase) et s pour les multilignes
	...
	fetch('Functions/funcFetchedJS.php', {
		...
		.then(function (status) {
			...
			if (status[0].success) {
				...
				alert(
					"Enregistrement des données réussies!! \n =>  finaliser l\'opération en chargeant l\'image acceptée....",
				);
				successFetch.innerHTML =
					"Enregistrement des données réussies <br> => finaliser l\'opération en chargeant ci dessous l\'image acceptée....";
...
imageUpload.addEventListener('change', function () {
	...
	if (imageUploadFileSize != imageSelectedFileSize) {
		errorMessage.innerHTML =
			'La taille du fichier ne correspond pas avec le prévisualisé';
		return false;
	}
