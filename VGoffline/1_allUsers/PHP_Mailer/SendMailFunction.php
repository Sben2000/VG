...
//Path pour passage des variables d'environnement lors du dév (via dotenv)
define('__ROOTDB__', dirname(__DIR__)); //renvoi à la racine 
require_once __ROOTDB__ . "/DB/config.php";
...
function sendMail($mail, $subject, $recipient, $body)
{ //mise sous la forme de function qui sera appelée
    ...
    $mail->Username   = /*'user@example.com'*/ $_ENV['SMTP_Mailer_EMAIL']; //SMTP username ->notre compte mail utilisateur
    $mail->Password   = /*'secret'*/ $_ENV['SMTP_Mailer_PASS'];         //SMTP password ->donnée dans google/sécurité en ajoutant la vérif à deux étapes, mot de passe des applications/entrer le nom de l'appli/puis copier et coller ici le code sans espace
    ...
    $mail->setFrom($_ENV['SMTP_Mailer_EMAIL'], 'Vite&Go');/*('from@example.com', 'Mailer')*/ //apparaitra avec mon Mail avec le message posté en entête
