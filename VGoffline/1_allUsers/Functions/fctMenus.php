...
//function qui récupère sur la sélection unique du filtre maxPrice
function getMenusByMaxPriceOnly($selectedMaxPrice){
        //récupération de la connection BDD
    $conn = DBconnection();
    if(!$conn){
        return false;
    }
    //récupère les menus avec le prix <= au prix max choisi et la qté >0  et les tri du plus récent menu au plus ancien
    $sql = "
    SELECT menu.menu_id, menu.regime_id, menu.theme_id, menu.titre, menu.nombre_personne_minimum, menu.prix_par_personne, menu.description, menu.photo_menu, menu.quantite_restante,
     theme.theme_id,theme.libelle as theme,
    regime.regime_id, regime.libelle as regime
...
//function qui récupère la sélection du prix max du filtre priceRange pour en afficher les prix supérieurs
function getMenuUpperPrices($selectedPriceRange){
        //récupération de la connection BDD
    $conn = DBconnection();
    if(!$conn){
        return false;
...
//function qui récupère les  prix min et  max du filtre priceRange pour afficher les résultat de la plage
function getMenusByPriceRange($selectedPriceRangeMin, $selectedPriceRangeMax){
        //récupération de la connection BDD
    $conn = DBconnection();
    if(!$conn){
        return false;
    ...
    $query->bindParam(":selectedPriceRangeMin",$selectedPriceRangeMin, PDO::PARAM_INT);
        $query->bindParam(":selectedPriceRangeMax",$selectedPriceRangeMax, PDO::PARAM_INT);
    $query ->execute();
    $result = $query->fetchAll(PDO::FETCH_OBJ);
    //var_dump($result);
    return $result;
