...
require_once "model/platsAllerModel.php";
require_once "model/modFetchSelectAllerg.php";
require_once "model/modFetchSelectPlat.php";

/*******************************Affiche la liste des plats à jour dans index.php*************************************************** */
function indexAction(){
    $plats = latests_plats();//on assigne à $plats l'objet fetch par la function latests_plats
    require_once 'views/liste_platsAllerg.php';
}
...
if(isset($_POST["addButton"])){
    //on récupère les id du plat et de l'allergène à associer et FETCH par les JS respectifs
        ...
        $allergeneID = $_POST['allergeneChoosenID'];
 //on execute la function 
        $response = createAssoc($platID, $allergeneID);
}
...
//on récupère les allergènes via la function du model pour les afficher dans la liste déroulante
$allergenes = getAllergenes();
$contient = view($idAllergene, $idPlat);//function du Model qui affiche les éléments de la combinaison d'id sélectionnée 
...
if(isset($_POST["modifyButton"])){
    //on récupère la valeur ID assignée au plat concerné 
        $platID = $_POST['dishID'];
    //on récupère la valeur ID assignée à l'allergène à modifier 
        $allergIDtoReplace = $_POST['allergIDtoReplace'];    
    //on récupère la valeur du FETCH JS , l'id du nouvel allergène subistituant l'ancien
        $allergeneNewID = $_POST['allergeneChoosenID'];
    //Vérification récupération des ID concernés en décommentant ci dessous
    //echo ($platID . " " . $allergIDtoReplace  ." " .$allergeneNewID );
    
    //La function de mise à jour est jouée avec les arguments récupérés et la réponse fournie avec le résultat 
    $response = editAllergAssoc($platID, $allergIDtoReplace, $allergeneNewID);
    }
...
$idAllergene= $_GET['idAllergene'];
$idPlat= $_GET['idPlat'];
$response = destroy($idAllergene, $idPlat); 
